import { T as TSS_SERVER_FUNCTION, c as createServerFn } from "./server-BaoOPsda.mjs";
...
function computeActivityScore(repos) {
  const now = Date.now();
  const active = repos.filter((r) => !r.fork && !r.archived);
  if (!active.length) return 0;
  const recent = active.filter((r) => now - new Date(r.pushed_at).getTime() < 1e3 * 60 * 60 * 24 * 90).length;
  const veryRecent = active.filter((r) => now - new Date(r.pushed_at).getTime() < 1e3 * 60 * 60 * 24 * 30).length;
  const ratio = recent / active.length;
  return clamp(ratio * 70 + Math.min(veryRecent, 10) * 3);
}
function computeQualityScore(repos) {
...
function computeCommunityScore(user, repos) {
  ...
  return clamp(
    Math.log10(user.followers + 1) * 22 + Math.log10(stars + 1) * 18 + Math.log10(forks + 1) * 10
  );
}
function computeProfileScore(user, hasReadme) {
  let s = 0;
...
function computeLanguages(repos) {
  ...
  return Object.entries(counts).map(([name, value]) => ({ name, value: Math.round(value * 10) / 10 })).sort((a, b) => b.value - a.value).slice(0, 8);
}
...
function computeAll(user, repos, hasProfileReadme) {
  const breakdown = {
  ...
  const score = Math.round(
    breakdown.activity * 0.22 + breakdown.quality * 0.26 + breakdown.diversity * 0.14 + breakdown.community * 0.22 + breakdown.profile * 0.16
  );
  return { score, breakdown, rating: rateScore(score) };
}
const SEVERITY_WEIGHT = { important: 2, suggestion: 1, info: 0 };
function buildContext(user, repos, hasReadme, breakdown) {
  const own = repos.filter((r) => !r.fork);
  ...
  const languages = [...new Set(own.filter((r) => r.language).map((r) => r.language))];
...
const ALL_RULES = [...CORE_RULES, ...buildLanguageRules()];
function buildRecommendations(user, repos, hasReadme, breakdown) {
  ...
  return matched.sort((a, b) => SEVERITY_WEIGHT[b.severity] - SEVERITY_WEIGHT[a.severity]).slice(0, 8);
}
...
const analyzeUser_createServerFn_handler = createServerRpc({
  id: "99de2d9a4a035cd84ff66b062232a94fa841bee0f8ef394e703582f7fe8047ae",
  name: "analyzeUser",
  filename: "src/lib/github.functions.ts"
}, (opts) => analyzeUser.__executeServer(opts));
const analyzeUser = createServerFn({
  method: "GET"
}).inputValidator((input) => ({
  username: usernameSchema.parse(input.username)
})).handler(analyzeUser_createServerFn_handler, async ({
  data
}) => {
  const token = process.env.GITHUB_TOKEN;
  const userRes = await gh(`/users/${data.username}`, token);
  if (!userRes.ok) {
  ...
  const reposRes = await gh(`/users/${data.username}/repos?per_page=100&sort=updated`, token);
  const repos = reposRes.ok ? reposRes.data : [];
  const readmeRes = await gh(`/repos/${data.username}/${data.username}/readme`, token);
  const hasProfileReadme = readmeRes.ok;
  const {
    score,
    breakdown,
    rating
  } = computeAll(user, repos, hasProfileReadme);
  const languages = computeLanguages(repos);
  const topRepos = [...repos].filter((r) => !r.fork).sort((a, b) => b.stargazers_count - a.stargazers_count || b.forks_count - a.forks_count).slice(0, 6);
...
export {
  analyzeUser_createServerFn_handler
};
